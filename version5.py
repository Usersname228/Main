...
def format_search_results_with_favorite_detailed(results, query, user_id, search_type='currency'):
    ...
    if len(results) == 1:
        ...
        if search_type == 'stock':
            ...
            # Получаем реальные данные
            real_time_data = get_real_time_crypto_price(crypto['symbol'])
            ...
            if real_time_data:
                ...
                result_text += f"   💰 Цена: {real_time_data['price_rub']:,.0f}₽ (${real_time_data['price_usd']:,.2f})\n"
                
                if real_time_data['change_24h'] != 0:
                    result_text += f"   📊 24ч: {change_sign}{real_time_data['change_24h']:.1f}% {change_icon}\n"
                else:
                ...
                result_text = f"*{crypto['symbol']}* - {crypto['name']}\n"
                result_text += f"   💰 Цена: *данные временно недоступны*\n"
