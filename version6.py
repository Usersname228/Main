...
CHANNEL_USERNAME = "@nweproger"  # Ваш канал
CHANNEL_ID = -1002901750088  # ID вашего канала (убедитесь, что верный)
...
def check_subscription(user_id: int) -> bool:
    ...
    try:
        ...
        logger.info(f"Статус пользователя {user_id}: {chat_member.status}")
        
        # Проверяем статусы подписки
        valid_statuses = ['member', 'administrator', 'creator', 'restricted']
        
        logger.info(f"Все атрибуты chat_member: {dir(chat_member)}")
        logger.info(f"chat_member статус: {chat_member.status}")
        ...
        logger.error(f"Общая ошибка при проверке подписки пользователя {user_id}: {e}")
        return False
...
def create_subscription_keyboard():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("🔍 Проверить подписку", callback_data="check_subscription_main"),
        InlineKeyboardButton("ℹ️ О боте", callback_data="bot_info_before_subscription")
    )
    return keyboard

def create_main_menu_keyboard():
    """Создает основную клавиатуру после проверки подписки"""
    ...
    keyboard.add(button9, button10, button11, button12, button13)
    return keyboard
...
def check_user_access_full(user_id, chat_id, feature_name=""):
    """Полная проверка доступа: подписка → регистрация → портфель"""
...
Для использования *Финансового Бота* необходимо подписаться на наш канал!

*📢 Наш канал:* {CHANNEL_USERNAME}

*✨ Что вы получите в канале:*
• Эксклюзивные финансовые анализы
• Рекомендации по инвестициям
• Своевременные уведомления о рынке
...
• Персональному портфелю
• Умным уведомлениям
• Финансовому калькулятору

Нажмите кнопку ниже, чтобы подписаться, а затем проверьте подписку 👇
"""
        return False, subscription_text, 'subscription_required'
...
"""
        return False, registration_text, 'registration_required'
    
    # 3. Проверка портфеля (для функций, требующих портфель)
    portfolio_required_features = [
        "🏆 Топ валют", "📈 Криптовалюты", "📊 Аналитика РФ", 
        "🔍 Поиск валюты", "🔎 Поиск крипты", "📈 Поиск акций",
        "⭐ Избранное", "🔔 Мои уведомления", "🧮 Калькулятор",
        "Топ валют", "Криптовалюты", "Аналитика акций",
        "Поиск валюты", "Поиск криптовалюты", "Поиск акций",
        "Избранное", "Уведомления", "Калькулятор"
    ]
    
    has_portfolio = db.get_user_status(user_id)
    if feature_name in portfolio_required_features and not has_portfolio:
        portfolio_text = f"""
🚫 *ДОСТУП ОГРАНИЧЕН*
...
"""
        return False, portfolio_text, 'portfolio_required'
...
def send_welcome(message):
    ...
    db.add_user_action(user_id, "start_command")
    
    # Проверяем подписку
    is_subscribed = check_subscription(user_id)
    
    if not is_subscribed:
        user_states[chat_id] = 'subscription_check'
...
• Данные по российским акциям (MOEX)
• Персональный портфель инвестиций
• Умные уведомления о важных изменениях
• Финансовый калькулятор и конвертер

*🚀 Для доступа к функциям необходимо:*
1. Подписаться на наш канал {CHANNEL_USERNAME}
2. Пройти быструю регистрацию (3 вопроса)
3. Создать свой портфель
...
"""
    ...
    # Если подписка есть, проверяем регистрацию
    registration_completed = db.is_registration_completed(user_id)
    ...
    if not registration_completed:
        ...
        if not has_portfolio:
            ...
            welcome_text = """
*✅ Подписка и регистрация завершены!*
...
"""
            ...
            welcome_text = f"""
*✅ ВЫ ПОЛНОСТЬЮ АКТИВИРОВАЛИ БОТА!*
...
• Проверка подписки (кнопка ниже)
• Информация о боте

*После подписки и регистрации будут доступны:*
• 🏆 Топ валют
• 📈 Криптовалюты
• 📊 Аналитика компаний
• 🔍 Поиск валюты
...
*Команды:*
/start - Запустить бот / проверить подписку
/help - Справка
/check - Проверить подписку

_Начните с подписки на канал!_
"""
        bot.send_message(
            chat_id,
            help_text,
...
1. Пройти регистрацию (команда /start)
2. Создать портфель
3. Добавить хотя бы один актив
...
def check_subscription_command(message):
    ...
    save_user_info(message)
    db.add_user_action(user_id, "check_subscription_command")
    ...
    if is_subscribed:
        bot.send_message(
            ...
            "Вы можете использовать все функции бота после регистрации и создания портфеля.\n\n"
        ...
        bot.send_message(
            ...
            "Пожалуйста, подпишитесь на канал, чтобы получить доступ к функциям бота.",
            parse_mode='Markdown',
            reply_markup=create_subscription_keyboard()
        )
...
def handle_favorites_command(message):
    ...
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Избранное")
    
    if not has_access:
        if access_type == 'subscription_required':
            bot.send_message(
                chat_id,
                error_message,
                parse_mode='Markdown',
        ...
        elif access_type == 'registration_required':
            bot.send_message(
                chat_id,
                error_message,
                parse_mode='Markdown'
            )
        elif access_type == 'portfolio_required':
            bot.send_message(
                chat_id,
                error_message,
                parse_mode='Markdown',
...
def handle_my_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Портфель")
...
def handle_top_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Топ валют")
...
def handle_crypto_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Криптовалюты")
...
def handle_analysis_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Аналитика акций")
...
def handle_search_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Поиск валюты")
...
def handle_cryptosearch_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Поиск криптовалюты")
...
def handle_stocksearch_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Поиск акций")
...
def handle_calculator_command(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Калькулятор")
...
@bot.callback_query_handler(func=lambda call: call.data == "check_subscription_main")
def handle_subscription_check(call):
    """Обработчик проверки подписки из меню"""
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    
    try:
        logger.info(f"Обработка проверки подписки от пользователя {user_id}")
        ...
        if is_subscribed:
            ...
            keyboard = InlineKeyboardMarkup(row_width=2)
            keyboard.add(
                InlineKeyboardButton("✅ Начать регистрацию", callback_data="start_registration_after_sub"),
                InlineKeyboardButton("📢 Наш канал", url=f"https://t.me/{CHANNEL_USERNAME[1:]}")
            )
            
            new_text = "✅ *ВЫ ПОДПИСАНЫ НА КАНАЛ!*\n\n" \
                      "*🎉 Отлично! Теперь вы можете продолжить:*\n" \
                      "1. Пройти быструю регистрацию (3 вопроса)\n" \
                      "2. Создать свой портфель\n" \
                      "3. Получить доступ ко всем функциям бота\n\n" \
                      "*✨ Ваши возможности после регистрации:*\n" \
                      "• Отслеживание инвестиций\n" \
                      "• Актуальные финансовые данные\n" \
                      "• Умные уведомления\n" \
                      "• Финансовый калькулятор"
            
            logger.info(f"Пользователь {user_id} подписан, обновляю сообщение")
            ...
            bot.answer_callback_query(call.id, text="✅ Вы подписаны на канал!")
            
        else:
            # Пользователь не подписан
            keyboard = create_subscription_keyboard()
            
            new_text = "❌ *ВЫ НЕ ПОДПИСАНЫ НА КАНАЛ!*\n\n" \
                      f"Пожалуйста, подпишитесь на канал {CHANNEL_USERNAME}\n\n" \
                      "*После подписки:*\n" \
                      "1. Нажмите '🔍 Проверить подписку'\n" \
                      "2. Пройдите регистрацию (3 вопроса)\n" \
                      "3. Создайте портфель\n" \
                      "4. Получите доступ ко всем функциям!"
            
            logger.info(f"Пользователь {user_id} не подписан, обновляю сообщение")
        ...
        logger.error(f"Ошибка при обработке проверки подписки пользователя {user_id}: {e}")
        bot.answer_callback_query(
            call.id, 
            text="⚠️ Произошла ошибка при проверке подписки. Попробуйте позже."
        )

@bot.callback_query_handler(func=lambda call: call.data == "start_registration_after_sub")
def handle_start_registration_after_sub(call):
    """Начало регистрации после проверки подписки"""
    user_id = call.from_user.id
    chat_id = call.message.chat.id
...
@bot.callback_query_handler(func=lambda call: call.data == "bot_info_before_subscription")
def handle_bot_info_before_subscription(call):
    """Информация о боте до подписки"""
    user_id = call.from_user.id
    chat_id = call.message.chat.id
...
1. Подписка на канал {CHANNEL_USERNAME}
2. Быстрая регистрация (3 вопроса)
3. Создание портфеля (добавление первого актива)
...
_Подпишитесь на канал, чтобы начать использовать бота!_
"""
    ...
    bot.answer_callback_query(call.id, text="ℹ️ Информация о боте")
...
@bot.message_handler(func=lambda message: message.text in [
    '🏆 Топ валют', '📈 Криптовалюты', '📊 Аналитика РФ',
    '🔍 Поиск валюты', '🔎 Поиск крипты', '📈 Поиск акций',
    '📊 Мой портфель', '🔔 Мои уведомления', '⭐ Избранное',
    '🧮 Калькулятор', '📨 Связь с админом', 'ℹ️ О боте'
])
def handle_main_menu_buttons(message):
    """Обработчик всех основных кнопок меню"""
    user_id = message.from_user.id
    chat_id = message.chat.id
    
    button_text = message.text
    
    # Проверяем полный доступ
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, button_text)
    ...
    # Если доступ есть, обрабатываем кнопку
    if button_text == '🏆 Топ валют':
        handle_top_currencies(message)
    elif button_text == '📈 Криптовалюты':
        handle_crypto_rates(message)
    elif button_text == '📊 Аналитика РФ':
        handle_analysis_button(message)
    elif button_text == '🔍 Поиск валюты':
        handle_currency_search(message)
    elif button_text == '🔎 Поиск крипты':
        handle_crypto_search(message)
    elif button_text == '📈 Поиск акций':
        handle_stock_search(message)
    elif button_text == '📊 Мой портфель':
        handle_portfolio(message)
    elif button_text == '🔔 Мои уведомления':
        handle_alerts(message)
    elif button_text == '⭐ Избранное':
        handle_favorites_button(message)
    elif button_text == '🧮 Калькулятор':
        handle_calculator_button(message)
    elif button_text == '📨 Связь с админом':
        handle_contact_admin(message)
    elif button_text == 'ℹ️ О боте':
        handle_about_with_subscription_check(message)

def handle_about_with_subscription_check(message):
    """Обработчик кнопки 'О боте' с проверкой подписки"""
    user_id = message.from_user.id
    chat_id = message.chat.id
...
def handle_alerts(message):
    ...
    # Проверка полного доступа
    has_access, error_message, access_type = check_user_access_full(user_id, chat_id, "Уведомления")
...
def handle_contact_admin(message):
    ...
    if not is_subscribed:
        bot.send_message(
            chat_id,
            f"❌ *Для связи с администратором необходимо подписаться на канал {CHANNEL_USERNAME}*",
            parse_mode='Markdown',
            reply_markup=create_subscription_keyboard()
        )
        return
...
def handle_create_portfolio(message):
    ...
    if not is_subscribed:
        bot.send_message(
            chat_id,
            f"❌ *Для создания портфеля необходимо подписаться на канал {CHANNEL_USERNAME}*",
            parse_mode='Markdown',
            reply_markup=create_subscription_keyboard()
        )
        return
...
def handle_cancel(message):
    ...
    if current_state in ['contact_mode', 'search_currency', 'search_crypto', 'search_stock',
                         'portfolio_menu', 'alerts_menu', 'calculator_menu', 'favorites_menu',
                         'converter_from', 'converter_to', 'converter_amount',
                         'profit_calc_initial', 'asset_value_calc', 'compound_calc', 'loan_calc_type']:
        user_id = message.from_user.id
        
        # Проверяем подписку
        is_subscribed = check_subscription(user_id)
        if not is_subscribed:
            user_states[message.chat.id] = 'subscription_check'
            bot.send_message(
                message.chat.id,
                f"❌ *Для использования бота необходимо подписаться на канал {CHANNEL_USERNAME}*",
                parse_mode='Markdown',
...
def handle_registration_cancel_choice(message):
    ...
    if not is_subscribed:
        bot.send_message(
            chat_id,
            f"❌ *Для продолжения регистрации необходимо подписаться на канал {CHANNEL_USERNAME}*",
            parse_mode='Markdown',
            reply_markup=create_subscription_keyboard()
        )
        return
...
def handle_registration_answers(message):
    ...
    # Проверяем подписку перед обработкой ответов
    is_subscribed = check_subscription(user_id)
    if not is_subscribed:
        bot.send_message(
            chat_id,
            f"❌ *Для завершения регистрации необходимо подписаться на канал {CHANNEL_USERNAME}*",
            parse_mode='Markdown',
            reply_markup=create_subscription_keyboard()
        )
        return
...
if __name__ == "__main__":
    ...
    print(f"🔒 Доступ: Подписка → Регистрация → Портфель → Все функции")
    ...
    print("🚀 Полный доступ: Подписка → регистрация → портфель → все функции")
