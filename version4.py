...
def add_to_favorites_function(chat_id, user_id, symbol, asset_type, name):
    ...
    if success:
        bot.send_message(
            chat_id,
            f"✅ *{symbol}* добавлен(а) в избранное!",
            parse_mode='Markdown',
...
def show_favorites(chat_id, user_id):
    ...
    for fav in favorites:
        by_type[fav['asset_type']].append(fav)
...
def show_favorites_with_prices(chat_id, user_id):
    """Показать избранное с текущими ценами"""
    favorites = db.get_favorites(user_id)
    ...
    favorites_text = f"⭐ *ИЗБРАННОЕ С КОТИРОВКАМИ*\n\n"
    favorites_text += f"_Обновлено: {datetime.now().strftime('%d.%m.%Y %H:%M')}_\n\n"
    
    for i, fav in enumerate(favorites, 1):
        # Получаем текущую цену
        current_price = get_current_price(fav['symbol'], fav['asset_type'])
        
        favorites_text += f"*{i}. {fav['symbol']}* - {fav['name']}\n"
        favorites_text += f"Тип: {fav['asset_type']}\n"
        
        if current_price:
            favorites_text += f"Текущая цена: *{current_price:,.2f}₽*\n"
            
            # Для сравнения с предыдущим днем (симуляция)
            ...
            favorites_text += f"Изменение: {change_sign}{change:,.2f} ({change_sign}{change_percent:.2f}%) {change_icon}\n"
        else:
            favorites_text += "Цена: *недоступна*\n"
        
        # Кнопка управления
        keyboard = create_manage_favorite_keyboard(fav['id'], fav['symbol'])
        
        if i == 1:
            # Для первого элемента отправляем с кнопками
            bot.send_message(
                chat_id,
                favorites_text,
                parse_mode='Markdown',
            ...
            # Для остальных отправляем без кнопок
            bot.send_message(
                chat_id,
                favorites_text,
                parse_mode='Markdown'
    ...
    # Отправляем итоговое сообщение с главными кнопками
    if favorites_text:
        bot.send_message(
            chat_id,
            favorites_text,
...
def format_search_results_with_favorite(results, query, user_id, search_type='currency'):
    """Форматирование результатов поиска с кнопкой добавления в избранное"""
    if not results:
    ...
    if len(results) == 1:
        result_text = f"*🔍 Найдена {search_type}*\n\n"
        
        # Для одного результата показываем подробно с кнопкой
        if search_type == 'currency':
            ...
            if currency['nominal'] > 1:
                ...
                result_text += f"💰 {currency['nominal']} ед.: *{currency['value']:.4f}₽*\n"
                result_text += f"📊 1 ед.: *{value_per_unit:.4f}₽*\n"
            else:
                result_text += f"💰 *{currency['value']:.4f}₽*\n"
            
            result_text += f"📊 Изменение: {change_sign}{currency['change']:.4f} {change_icon}\n"
            ...
            if currency['code'] in POPULAR_CURRENCIES:
                ...
                result_text += f"🏳️ Символ: {flag} {symbol}\n"
            ...
            return None  # Возвращаем None, т.к. отправили отдельно
            ...
            result_text += f"🇺🇸 Цена: ${crypto['price_usd']:,.4f}\n"
            result_text += f"🇷🇺 Цена: {crypto['price_rub']:,.0f}₽\n"
            
            if crypto['change_24h'] != 0:
                result_text += f"24ч: `{change_sign}{crypto['change_24h']:.1f}%` {change_icon}\n"
            else:
                result_text += f"24ч: `0.0%` ➡️\n"
            ...
            result_text += f"\n_Данные: CoinGecko, время: {current_time}_"
            return None
            ...
            result_text += f"💰 Цена: {stock['price']:,.2f}₽\n"
            result_text += f"📈 Изменение: {change_sign}{stock['change']:,.2f} ({change_sign}{stock['change_percent']:.2f}%) {change_icon}\n"
            ...
            result_text += f"💎 Капитализация: {cap_str}\n"
...
def handle_favorites_command(message):
    ...
    if len(args) > 1:
        ...
        if subcommand == 'update':
            show_favorites_with_prices(chat_id, user_id)
...
Здесь вы можете:
• Быстро получать котировки любимых активов
• Управлять списком избранного
• Добавлять активы в портфель прямо из избранного
• Настраивать уведомления

*Как добавить в избранное:*
1. Найдите актив через поиск
...
def handle_search_query(message):
    ...
    query = message.text
    results = search_currency(query)
    formatted_results = format_search_results_with_favorite(results, query, user_id, 'currency')
...
def handle_crypto_search_query(message):
    ...
    query = message.text
    results = search_crypto(query)
    formatted_results = format_search_results_with_favorite(results, query, user_id, 'crypto')
...
def handle_stock_search_query(message):
    ...
    query = message.text
    results = search_stock(query)
    formatted_results = format_search_results_with_favorite(results, query, user_id, 'stock')
...
def handle_favorites_callback(call):
    ...
    if call.data == 'favorites_show_all':
        ...
        if favorite:
            # Получаем текущую цену
            current_price = get_current_price(favorite['symbol'], favorite['asset_type'])
            
            if current_price:
                # Для валюты
                if favorite['asset_type'] == 'currency':
                    ...
                    try:
                        ...
                        if favorite['symbol'] in data['Valute']:
                            valute = data['Valute'][favorite['symbol']]
                            previous = valute['Previous']
                            change = valute['Value'] - previous
                            ...
                            quote_text = f"""
*⭐ КОТИРОВКА ИЗБРАННОГО*

*{favorite['symbol']}* - {favorite['name']}

💰 *Текущий курс:*
• {valute['Nominal']} {favorite['symbol']} = *{valute['Value']:.4f}₽*
• 1 {favorite['symbol']} = *{valute['Value']/valute['Nominal']:.4f}₽*

📊 *Изменение за день:*
• Абсолютное: {change_sign}{change:.4f}₽
• Процентное: {change_sign}{change_percent:.2f}%
• Статус: {change_icon}
...
"""
                ...
                # Для крипты
                    ...
                    # Ищем крипту по символу
                    crypto_id = None
                    crypto_info = None
                    
                    for cid, info in POPULAR_CRYPTOCURRENCIES.items():
                        if info['symbol'] == favorite['symbol']:
                            crypto_id = cid
                            crypto_info = info
                            break
...
{crypto_info['emoji']} *{crypto_info['name']} ({crypto_info['symbol']})*
...
• {change_sign}{change_24h:.1f}% {change_icon}
...
"""
                ...
                # Для акций
                elif favorite['asset_type'] == 'stock':
                    # Используем демо-данные для акций
                    if favorite['symbol'] in RUSSIAN_STOCKS:
                        stock_info = RUSSIAN_STOCKS[favorite['symbol']]
                        price = random.uniform(10, 15000)
                        change = random.uniform(-price*0.05, price*0.05)
                        change_percent = (change / price) * 100
                        volume = random.randint(10000, 5000000)
...
📊 *Информация:*
• Сектор: {stock_info['sector']}
• Биржа: {stock_info['market']}

💰 *Текущая цена:*
• *{price:,.2f}₽* за акцию

📈 *Изменение:*
• Абсолютное: {change_sign}{change:,.2f}₽
• Процентное: {change_sign}{change_percent:.2f}%
• Статус: {change_icon}
...
🏛️ *Данные Московской биржи (MOEX)*
🕒 Обновлено: {datetime.now().strftime('%d.%m.%Y %H:%M')}
"""
                ...
                bot.answer_callback_query(call.id, "❌ Не удалось получить цену")
        else:
        ...
        if favorite:
            ...
            user_temp_data[user_id]['asset_type'] = favorite['asset_type']
...
• Эффективная ставка: {effective_rate:+.2f}% годовых
...
if __name__ == "__main__":
    ...
    print(f"⭐ Новая функция: Избранное с быстрым доступом к активам")
    print(f"📊 Добавление в избранное при поиске валют, крипты и акций")
    print(f"💾 Избранное хранится в базе данных")
    ...
    print(f"🧮 Новый модуль: Финансовый калькулятор с 5 инструментами")
